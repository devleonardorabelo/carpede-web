...
export const TextInput = ({ label, action, type, name, error }) => {
    return (
        <>
            <div
                className={
                    error && error.input === name ? 'textInput textInputErrored' : 'textInput'
                }>
                <label>{label}</label>
                <input type={type ? type : 'text'} onChange={action} />
            </div>
